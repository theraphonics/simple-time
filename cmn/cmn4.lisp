...
(cmn (staff treble  (c5 (rq 1/6)) (c5 (rq 5/6)) (a4 (rq 2/7)) (a4 (rq 5/7)) (c5 (rq 5/6)) (c5 (rq 1/6)) (a4 (rq 5/7)) (a4 (rq 2/7))))
;; (bug from Bill Sack)
|#
...
(defun insert-measure-numbers (score)
  ...
  (let* ((bottom-staff (first (last (staves (first (systems score))))))
    ...
    (loop for obj in (cdr (staff-data bottom-staff)) do
	...
	(if (and (rest-p obj)
		 (eq (rest-mark obj) :measurerest)
		 (store-data obj))
	    (let ((mn (find-if #'(lambda (n) (and (listp n) (eq (first n) :measure-number))) (store-data obj))))
	      (if mn
		  (incf bar-num (1- (second mn))))))))
...
;;; someday: :old-style layout staff omitted on 1st and clef missing upon appearance
...
(defun new-staff (score i staff anewdata x0 y0 x1 &optional (nx0 0) alines aname amarks abracket abrace cline)
  (if (not (eq (justification staff) :new-style))
      (progn
	(if (or (not (eq (justification staff) :old-style))
		(let* ((line-onset (tld-time (first cline)))
		       (line-offset (tld-time (first (last cline)))))
		  (not (null (find-line-intersection (times staff) line-onset line-offset)))))
	    (push 
			...
			(let ((cur-time (tld-time td)) ;onset time of the current entity
			  ...
			  (if (aref active-staffs i)
				...
				(let* ((cur-obj (first (aref staffs i)))
				  ...
				  ;; MDE look for new-staff-name slot of note class and 
				  ;; change staff name if present 
				  (when (and (or (note-p cur-obj) 
						 (rest-p cur-obj)) 
					     (new-staff-name cur-obj)) 
				    (setf (aref staff-names i) 
					  (let ((new-text (copy (aref staff-names i)))) ; preserve font info 
					    (setf (letters new-text) 
						  (if (use-abbreviated-staff-names score) 
						      (abbreviation-of-instrument-name 
						       (new-staff-name cur-obj)) 
						    (new-staff-name cur-obj))) 
					    new-text)))
